...
from app.api.v1.administracion.globalClienteProveedor.schema_clientes_global import ClienteOutShort, UbicacionOut, ProveedorOut
...
@router_clientes_global.get("/mostrar-proveedores", response_model=List[ProveedorOut], status_code=status.HTTP_200_OK)
async def mostrar_proveedores(db: AsyncSession = Depends(get_session)):
    query = (
        select(
            GlobalProveedor.id,
        ...
        .order_by(GlobalProveedor.razon_social.asc())
    )
