...
class ServicioChips(Base):
    __tablename__ = "tabla_chips_servicios_monitoreo"
    ...
    chip_id: Mapped[Optional[int]] = mapped_column(ForeignKey("administracion.tabla_chips_inventario_monitoreo.id"), nullable=True, index=True)
    ...
    cliente: Mapped[Optional["GlobalCliente"]] = relationship(back_populates="servicio_chips")
    ubicacion: Mapped[Optional["TablaUbicacionesMonitoreo"]] = relationship(back_populates="servicio_chips")
    chip: Mapped[Optional["InventarioChips"]] = relationship(back_populates="servicio_chips")