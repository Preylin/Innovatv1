...
export type ComboboxOption = {
  value: string; // ID original convertido a string
  label: string; // Texto a mostrar (Razón social, Ubicación, etc.)
};
...
export function CustomCombobox({
  ...
  // Sincroniza el input de texto si cambia el valor externamente o al seleccionar
  useEffect(() => {
    setQuery(value ? value.label : "");
  }, [value]);

  // Filtrado simple e inmediato en tiempo real
  const filteredItems = useMemo(() => {
    const searchTerm = query.toLowerCase().trim();
    if (!searchTerm || (value && value.label === query)) return items;
  ...
  // Cierra el menú desplegable si se hace clic fuera del componente completo
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (containerRef.current && !containerRef.current.contains(event.target as Node)) {
        setIsOpen(false);
        // Si no se guardó una opción válida, revierte el texto al label original
        setQuery(value ? value.label : "");
      }
    }
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      <div className="relative flex items-center">
        ...
        {/* Acciones en el Input: Borrar o Desplegar */}
      ...
      {/* Menú Desplegable flotante simulado */}
      {isOpen && (
