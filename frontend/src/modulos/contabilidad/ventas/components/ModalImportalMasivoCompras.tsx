...
const HistorialComprasImportSchema = z.object({
...
type HistorialComprasImport = z.infer<typeof HistorialComprasImportSchema>;
...
export default function HistorialComprasImportMasivaExcel({
  open,
  onClose,
}: {
  open: boolean;
  ...
  const parseFile = async (file: File) => {
    ...
    try {
      ...
      const validRows: HistorialComprasImport[] = [];
      ...
      worksheet.eachRow((row, rowNumber) => {
        ...
        const result = HistorialComprasImportSchema.safeParse(rowData);
        if (!result.success) {
          rowErrors.push({
            row: rowNumber,
  ...
  const mutation = useMutation<void, ApiError, FormData>({
    mutationFn: (fd) =>
      api.post("/contabilidad/compras/importar-ventas-excel", fd, {
