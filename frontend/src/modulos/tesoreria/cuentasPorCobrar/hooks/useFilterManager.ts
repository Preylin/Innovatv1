...
import {
  FilterManagerIntelligence,
  type DataGeneral,
  type SaldosFinancieros,
} from "./FilterManagerIntelligent";
...
export function useFilterManager(
  ...
  const dataNormalizada = useMemo<DataGeneral[]>(() => {
    ...
    return apiData
      .filter((item) => {
        ...
        return monto_total - monto_pagado > 0;
      })
      .map((item, index) => {
        ...
        return {
          ...
          dias: FilterManagerIntelligence.calcularTextoVencimiento(
  ...
  const brain = useMemo(
    () => new FilterManagerIntelligence(dataNormalizada, saldosApi),
    [dataNormalizada, saldosApi],
  );
  const datosFiltrados = useMemo(
    () => brain.filtrarPorRangoFecha(rangoFechas),
    [brain, rangoFechas],
  );

  const cuentasCobrar = useMemo(
    () => datosFiltrados.obtenerCuentasPorCobrar(),
    [datosFiltrados],
  );
  const cuentasPagar = useMemo(
    () => datosFiltrados.obtenerCuentasPorPagar(),
    [datosFiltrados],
  );
  ...
  const totalesCobrar = useMemo(
    () => datosFiltrados.calcularTotalesGrupo(cuentasCobrar),
    [datosFiltrados, cuentasCobrar],
  );
  const totalesPagar = useMemo(
    () => datosFiltrados.calcularTotalesGrupo(cuentasPagar),
    [datosFiltrados, cuentasPagar],
  );
  ...
  const exportarGrupoAExcel = useCallback(
    async (dataGrupo: DataGeneral[], tituloReporte: string) => {
  ...
  return {
    ...
    toggleCheck,
    exportarCobros: () =>
      exportarGrupoAExcel(cuentasCobrar, "Cuentas_Por_Cobrar"),
