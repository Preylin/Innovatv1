...
const ChipImportSchema = z.object({
  ...
  proveedor: z.string().min(3, "proveedor requerido"),
});

type ChipImport = z.infer<typeof ChipImportSchema>;
const EXPECTED_HEADERS = ["ruc", "proveedor"];

/* ====================================================== 
   2️⃣ Componente UI
   ====================================================== */
export default function ProveedorListImportMasiva({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const mutation = useMutation<void, ApiError, FormData>({
    mutationFn: (fd) => api.post("/proveedoresGerenciaInicio/import", fd, {
