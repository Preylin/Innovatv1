...
import type { ProManagerData } from "../model/ManagerData";
...
import ActualizarRegistroPro from "./actualizar-registro-pro";
import { useDeletePro, useUpdateProEstado } from "../model/api/pro-api";
...
const dateFilterFn: FilterFn<ProManagerData> = (row, columnId, filterValue) => {
...
const EstadoCeldaInteractiva: FC<EstadoCeldaInteractivaProps> = ({
  ...
  const { mutateAsync, isPending } = useUpdateProEstado(rowId);
...
function TablaMostrarRegistrosPro({ data }: { data: ProManagerData[] }) {
  const { mutateAsync, isPending } = useDeletePro();
  ...
  const columns = useMemo<ColumnDef<ProManagerData, any>[]>(
    () => [
      {
        accessorKey: "key",
        size: 25,
            ...
            <Drawer direction="right">
              ...
              <DrawerContent>
                <ActualizarRegistroPro
                  ...
                  adicional={info.row.original.adicional}
                />
              </DrawerContent>
            </Drawer>
  ...
  const columnsExcel = [
    ...
    { header: "Ruc", key: "nro_documento", width: 20 },
    { header: "Cliente", key: "razon_social", width: 20 },
    { header: "Ubicación", key: "ubicacion", width: 25 },
    { header: "Inicio", key: "fecha_inicio", width: 10 },
    { header: "Fin", key: "fecha_fin", width: 10 },
    { header: "Fact. rel.", key: "fact_relacionada", width: 10 },
    { header: "Estado", key: "estado", width: 13 },
    { header: "Adicional", key: "adicional", width: 10 },
  ];

  return (
    <div className="">
      <TableBaseFuzzyCntasPorCobrar<ProManagerData>
        data={data}
        columns={columns}
        fuzzyFilter={fuzzyFilter}
        columFiltersInitialValue={columnFilters}
        cantidadFilas={15}
        excelFileName="Registro de servicios Pro"
