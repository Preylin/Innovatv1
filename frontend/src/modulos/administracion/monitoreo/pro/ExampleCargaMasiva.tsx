...
const HistorialProSchema = z.object({
  ...
  fecha_fin: z
    .union([z.date(), z.string().transform((v) => new Date(v))])
    .pipe(z.date()),
  fact_relacionada: z.string().optional().nullable(),
  estado: z.string().optional(),
  adicional: z.string().optional().nullable(),
});

type HistorialPro = z.infer<typeof HistorialProSchema>;
...
export default function HistorialProMasivaExcel({
  open,
  onClose,
}: {
  open: boolean;
  ...
  const parseFile = async (file: File) => {
    ...
    try {
      ...
      worksheet.eachRow((row, rowNumber) => {
        ...
        const result = HistorialProSchema.safeParse(rowData);
        if (!result.success) {
          rowErrors.push({
            row: rowNumber,
  ...
  const mutation = useMutation<void, ApiError, FormData>({
    mutationFn: (fd) =>
      api.post("/servicio-pro/importar", fd, {
