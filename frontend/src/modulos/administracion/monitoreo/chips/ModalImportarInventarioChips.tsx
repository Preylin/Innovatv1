...
const HistoriaInventarioChipsSchema = z.object({
...
type HistoriaInventarioChips = z.infer<typeof HistoriaInventarioChipsSchema>;
...
export default function HistoriaInventarioChipsMasivaExcel({
  open,
  onClose,
}: {
  open: boolean;
  ...
  const parseFile = async (file: File) => {
    ...
    try {
      ...
      const validRows: HistoriaInventarioChips[] = [];
      ...
      worksheet.eachRow((row, rowNumber) => {
        ...
        const result = HistoriaInventarioChipsSchema.safeParse(rowData);
        if (!result.success) {
          rowErrors.push({
            row: rowNumber,
  ...
  const mutation = useMutation<void, ApiError, FormData>({
    mutationFn: (fd) =>
      api.post("/servicio-inventario-chips/importar", fd, {
  ...
  return (
    <Modal
      title="Importación Masiva Inventario Chips"
      open={open}
