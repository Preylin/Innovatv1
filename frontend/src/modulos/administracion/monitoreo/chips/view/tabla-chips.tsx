...
import type { ChipsManagerData } from "../model/ManagerData";
...
import ActualizarRegistroChips from "./actualizar-registro-chips";
import { AlertDialogDestructive } from "#components/ui/AlertDialogDestructive";
import {
  useDeleteChipServicio,
  useUpdateChipServicioEstado,
} from "../model/api/chips-api";
...
const dateFilterFn: FilterFn<ChipsManagerData> = (
  row,
  columnId,
  filterValue,
) => {
...
const EstadoCeldaInteractiva: FC<EstadoCeldaInteractivaProps> = ({
  ...
  const { mutateAsync, isPending } = useUpdateChipServicioEstado(rowId);
...
function TablaMostrarRegistrosChips({ data }: { data: ChipsManagerData[] }) {
  const { mutateAsync, isPending } = useDeleteChipServicio();
  ...
  const columns = useMemo<ColumnDef<ChipsManagerData, any>[]>(
    () => [
      {
        accessorKey: "key",
        size: 25,
  ...
  return (
    <div className="">
      <TableBaseFuzzyCntasPorCobrar<ChipsManagerData>
        data={data}
        columns={columns}
...
export default TablaMostrarRegistrosChips;
