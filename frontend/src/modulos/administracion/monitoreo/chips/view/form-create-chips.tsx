...
import type { ChipsCreateApiType } from "../model/api/chips-schema";
import { useChipInventarioList } from "../model/api/chips-inventario-api";
import { useRegistrarChipServicio } from "../model/api/chips-api";


const ChipsCreateSchema = z.object({
  cliente_id: z.number().min(1, "Requerido"),
  ubicacion_id: z.number().min(1, "Requerido"),
  chip_id: z.number().min(1, "Requerido"),
...
export function FormCreateChips() {
  const [fecha1, setFecha1] = useState(false);
  const [fecha2, setFecha2] = useState(false);
  const { mutateAsync } = useRegistrarChipServicio();
  const { data: chipsApi, isLoading: isLoadingChips } =
  ...
  const chipsOptions = useMemo<ComboboxOption[]>(() => {
    if (!chipsApi) return [];
    return chipsApi.map((chip) => ({
      value: String(chip.id),
      label: chip.numero_chip,
    }));
  }, [chipsApi]);
  ...
  const form = useForm({
    ...
    validators: {
      onSubmit: ChipsCreateSchema,
    },
    onSubmit: async ({ value, formApi }) => {
      try {
        const payload: ChipsCreateApiType = {
  ...
  return (
    <div className="flex flex-row flex-wrap gap-2 w-full max-w-md overflow-auto max-h-[calc(100vh-270px)]">
      <div className="p-6 w-full max-w-2xl mx-auto bg-background rounded-xl border shadow-sm">
        <h2 className="text-xl font-bold mb-6">Registrar Servicio Chips</h2>
        ...
        >
          ...
          <form.Field
            ...
            children={(field) => (
              <Field>
                <FieldLabel>Numero de chip</FieldLabel>
                <FieldContent>
                  {isLoadingChips ? (
                    <Button
                      variant="outline"
                    ...
                    <CustomComboboxForm
                      ...
                      onBlur={field.handleBlur}
                      onValueChange={(val) => field.handleChange(Number(val))}
                      emptyText="No se encontraron chips."
                    />
                  )}
                  {field.state.meta.isTouched &&
                    field.state.meta.errors?.length > 0 && (
