...
const HistorialChipsSchema = z.object({
...
type HistorialChips = z.infer<typeof HistorialChipsSchema>;
...
export default function HistorialChipsMasivaExcel({
  open,
  onClose,
}: {
  open: boolean;
  ...
  const parseFile = async (file: File) => {
    ...
    try {
      ...
      const validRows: HistorialChips[] = [];
      ...
      worksheet.eachRow((row, rowNumber) => {
        ...
        const result = HistorialChipsSchema.safeParse(rowData);
        if (!result.success) {
          rowErrors.push({
            row: rowNumber,
  ...
  const mutation = useMutation<void, ApiError, FormData>({
    mutationFn: (fd) =>
      api.post("/servicio-chips/importar", fd, {
  ...
  return (
    <Modal
      title="Importación Masiva Chips"
      open={open}
