...
const HistorialSerivicioChipsUISchema = z.object({
  name: z.string().min(1, "Empresa requerida"),
  ubicacion: z.string().min(1, "Ubicación requerida"),
  numero: z.string().min(1, "Número de serie requerido"),
  operador: z.string().min(1, "Modelo requerido"),
...
type HistorialMCUISchemaType = z.infer<typeof HistorialSerivicioChipsUISchema>;
const EXPECTED_HEADERS = ['name', 'ubicacion', 'numero', 'operador', 'plan', 'inicio', 'fin', 'fact_rel', 'adicional', 'status'];

export default function HistorialSerivicioChipsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const parseFile = async (file: File) => {
    ...
    try {
      ...
      worksheet.eachRow((row, rowNumber) => {
        ...
        const result = HistorialSerivicioChipsUISchema.safeParse(rowData);
        if (!result.success) {
          rowErrors.push({ 
            row: rowNumber, 
  ...
  const mutation = useMutation<void, ApiError, FormData>({
    mutationFn: (fd) => api.post("/chipservicio/import", fd, {
  ...
  return (
    <Modal
      title="Importación Masiva de servicios de chips"
      open={open}
