...
import { useUpdateChipServicio } from "../../../../../api/queries/modulos/administracion/monitoreo/chipservicio/chipservicio.api";
import type { ChipServicioCreateApiType, ChipServicioOutApiType } from "../../../../../api/queries/modulos/administracion/monitoreo/chipservicio/chipservicio.api.schema";
import { ApiError } from "../../../../../api/normalizeError";
import { useQueryClient } from "@tanstack/react-query";


const CrearChipServicioUpdateUISchema = z.object({
...
export function useChipServicioFromCache(id: number): ChipServicioOutApiType | undefined {
  const qc = useQueryClient();
  const chipservicio = qc.getQueryData<ChipServicioOutApiType[]>(['chipservicio']);
  return chipservicio?.find((c) => c.id === id);
}


function ModalUpdateChipServicio({
  id,
  open,
  onClose,
}: {
  ...
  //Usar datos de cache
  const chipServicio = useChipServicioFromCache(id);

  // 2. Actualizar datos al api
    const { mutateAsync } = useUpdateChipServicio(id);
  ...
  const form = useForm({
    defaultValues: {
    name: chipServicio?.name || "",
    ubicacion: chipServicio?.ubicacion || "",
    numero: chipServicio?.numero || "",
    operador: chipServicio?.operador || "",
    plan: chipServicio?.toString() || "",
    inicio: chipServicio?.inicio || "",
    fin: chipServicio?.fin || "",
    fact_rel: chipServicio?.fact_rel || "",
    status: chipServicio?.status || 0,
    adicional: chipServicio?.adicional || "",
    },
    validators: {
      onSubmit: CrearChipServicioUpdateUISchema,
    },
                ...
                >
                  <InputSearch
                    ...
                    onChange={(val) => {
                      ...
                      // Lógica integrada: TypeScript ya sabe qué es 'field' aquí
          ...
          <Col span={24}>
            <form.Field name="status">
              {(field) => (
                ...
                >
                  <fieldset
                    className="border-2 border-slate-600 p-3 rounded-xs"
                  >
                    <legend
                    className="px-3"
                    >
...
export default ModalUpdateChipServicio;
