...
import type { WeatherManagerData } from "../model/ManagerData";
...
import ActualizarRegistroWather from "./actualizar-registro-weather";
import {
  useDeleteWeather,
  useUpdateWeatherEstado,
...
const dateFilterFn: FilterFn<WeatherManagerData> = (
  row,
  columnId,
  filterValue,
) => {
...
const EstadoCeldaInteractiva: FC<EstadoCeldaInteractivaProps> = ({
    ...
    const { mutateAsync, isPending } = useUpdateWeatherEstado(rowId);
            ...
            <FieldLabel htmlFor="renewed-plan">
              <Field orientation="horizontal">
                <FieldContent>
                  ...
                  <FieldDescription>
                    Servicio renovado. Importante: Debe realizar el nuevo
                    registro respectivo.
                  </FieldDescription>
                </FieldContent>
...
function TablaMostrarRegistrosWather({ data }: { data: WeatherManagerData[] }) {
  const { mutateAsync, isPending } = useDeleteWeather();
  ...
  const columns = useMemo<ColumnDef<WeatherManagerData, any>[]>(
    () => [
      {
        accessorKey: "key",
        size: 25,
  ...
  const columnsExcel = [
    ...
    { header: "Factura Rel.", key: "fact_relacionada", width: 10 },
  ...
  return (
    <div className="">
      <TableBaseFuzzyCntasPorCobrar<WeatherManagerData>
        data={data}
        columns={columns}
