...
import type { DataCruda } from "../types/movilesTypes";
import { Moviles } from "../utils/Moviles";
import isoToDDMMYYYY from "../../../../helpers/Fechas";

// Definimos el tipo de entrada para mayor seguridad
export const useMovilesData = (dataFromApi: DataCruda[] = []) => {
  return useMemo(() => {
    ...
    const dataTransformada = dataFromApi.map((item) => {
      ...
      return {
        ...item,
        fecha_compra: isoToDDMMYYYY(item.fecha_compra),
        montoPorDepreciar: item.valor -  item.monto_depresiado,
      };
    });

    // 2. Procesamos con la clase Personal
    const helper = new Moviles(dataTransformada);
