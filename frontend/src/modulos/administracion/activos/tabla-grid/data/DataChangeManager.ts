class DataChangeManager<T extends { id: number | string }> {
  ...
  getPendingPayload() {
    ...
    this.changes.forEach((change) => {
      // IMPORTANTE: Antes de enviar, eliminamos el campo 'saldo' 
      // ya que el servidor no lo reconoce en sus esquemas
      const { saldo, ...dataWithoutSaldo } = change.data as any;
      
      if (change.type === 'NEW') created.push(dataWithoutSaldo);
      else updates.push(dataWithoutSaldo);
    });
...
export default DataChangeManager;