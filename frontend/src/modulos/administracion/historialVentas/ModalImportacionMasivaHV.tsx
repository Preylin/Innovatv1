...
const EXPECTED_HEADERS = ["fecha", "descripcion", "categoria", "ruc", "cliente", "tipo", "serie", "numero", "subtotal", "igv", "total", "tc"];

export default function HistorialVentasImportMasiva({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const mutation = useMutation<void, ApiError, FormData>({
    mutationFn: (fd) => api.post("/historialVentas/import", fd, {
