  ...
  >
    <Flex align="center" justify="start" gap={8} style={{ height: "100%" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <Text type="secondary" className="block text-xs">Código: {item.codigo}</Text>
        <Text strong className="block text-xs">Stock actual: {item.stock_actual}</Text>
...
function MercaderiaStatus() {
  ...
  // Transformamos las categorías en un array para el virtualizador
  ...
  const rowVirtualizer = useVirtualizer({
    ...
    estimateSize: useCallback(() => 250, []), // Altura estimada de cada sección con carrusel
    overscan: 2,
  });

  if (isLoading) return <Skeleton active paragraph={{ rows: 10 }} className="p-6" />;
  if (isError) return <Alert type="error" title="Error al cargar estados de stock" showIcon />;
      ...
      >
        {rowVirtualizer.getVirtualItems().map((virtualRow) => {
          ...
          return (
            ...
            >
              <Title level={4} style={{ color: section.color, marginBottom: "16px" }}>
