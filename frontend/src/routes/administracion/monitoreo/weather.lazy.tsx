...
import type {
  WeatherCreateApiType,
  WeatherOutApiType,
  WeatherUpdateApiType,
} from "../../../api/queries/modulos/administracion/monitoreo/weather/weather.api.schema";
...
import { OptionStatusUI } from "./pro.lazy";
...
function ShowWeather() {
  ...
  const { data, isLoading, isError, isPending } = useWeatherList();
  const [searchParams, setSearchParams] = useState({
    field: "name",
    value: "",
  });
  const { mutate } = useDeleteWeather();
    ...
    <div className="flex flex-col gap-4 p-2 width-full">
      ...
      >
        ...
        <Col span={4}>
          ...
          <CreateWeatherUI open={open} onClose={handleClose} />
        </Col>
      </Row>
      ...
      <div className="flex flex-wrap gap-4 width-full">
        {filteredData.map((weather) => (
          ...
          >
            ...
            <Row>
              <Col span={24}>Dirección: {weather.ubicacion}</Col>
              <Col span={24}>Inicio: {isoToDDMMYYYY(weather.inicio)}</Col>
              <Col span={24}>
                <div className="text-pink-500">
                  Fin: {isoToDDMMYYYY(weather.fin)}
                </div>
              </Col>
              <Col span={24}>Fact. relacionada: {weather.fact_rel}</Col>
              {weather.adicional && (
                <Col span={24}>Información adicional: {weather.adicional}</Col>
              )}
            </Row>
            <Divider
            ...
            <Row>
              <Col span={18}>
                {weather.status !== null && (
                  <Col span={24}>
                    <OptionStatusUI status={weather.status} />
                  </Col>
                )}
              </Col>
              <Col span={3}>
                <ButtonUpdate onClick={() => handleOpenModal(weather.id)} />
              </Col>
              <Col span={3}>
                <Popconfirm
...
const WeatherUpdateUISchema = z.object({
  name: z
    .string()
    .min(1, "El nombre es requerido")
    .max(100, "Acepta 100 caracteres"),
...
export function useUsuarioFromCache(
  id: number | null
): WeatherOutApiType | undefined {
  const qc = useQueryClient();
  const pro = qc.getQueryData<WeatherOutApiType[]>(["weather"]);
...
function UpdateWeatherUI({
  ...
  const pro = useUsuarioFromCache(id);

  const { mutateAsync } = useUpdateWeather(id);
  const { message } = App.useApp();
  const { data } = useClientesList();
  const dataFromBackend: DependentOption[] = useMemo(
    () =>
  ...
  const form = useForm({
    ...
    validators: {
      onSubmit: WeatherUpdateUISchema,
    },
    onSubmit: async ({ value, formApi }) => {
      try {
        const payload: WeatherUpdateApiType = {
          ...value,
          status: Number(value.status),
        };
        await mutateAsync(payload);
